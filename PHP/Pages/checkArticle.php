            ...
            <?php while ($row = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <td>
                        ...
                        // Verificar si todas las evaluaciones están completas para este artículo
                        $stmtEval = $mysqli->prepare("
                            SELECT COUNT(*) AS incompletas 
                            FROM revisiones 
                            WHERE IDArticulo = ? 
                            AND (CalidadTecnica IS NULL OR CalidadTecnica IS NULL OR ValoracionGlobal IS NULL OR ArgumentosValoracion IS NULL OR ComentariosRevisor IS NULL)
                        ");
                        $stmtEval->bind_param("i", $idArticulo);
                        $stmtEval->execute();
                        $resEval = $stmtEval->get_result();
                        $estadoEval = $resEval->fetch_assoc();
                        $evaluacionesCompletas = $estadoEval["incompletas"] == 0;
                        ?>

                        <?php if ($row["EsContacto"]): ?>
                            <?php if (!$row["EnRevision"]): ?>
                                <a href="editArticle.php?id=<?= $idArticulo ?>"><button>Editar</button></a> 
                                <a href="deleteArticle.php?id=<?= $idArticulo ?>" onclick="return confirm('¿Estás seguro de eliminar este artículo?')"><button>Eliminar</button></a>
                            <?php else: ?>
                        ...
                        <?php if ($evaluacionesCompletas && $row["EnRevision"]): ?>
                            <br><a href="viewReviews.php?id=<?= $idArticulo ?>"><button>Ver Evaluaciones</button></a>
                        <?php endif; ?>
                    </td>
                </tr>
            <?php endwhile; ?>
