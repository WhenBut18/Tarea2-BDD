...
CREATE TABLE revisores (
    RutRev VARCHAR(12) PRIMARY KEY,
    Nombre VARCHAR(50) NOT NULL UNIQUE,
    Correo VARCHAR(150) NOT NULL UNIQUE
);

-- AÑADIR CARACTERISITCA DE REVISIOJ
...
CREATE TABLE revisiones (
    ...
    CONSTRAINT FK_revisiones_RutRev FOREIGN KEY (RutRev) REFERENCES revisores(RutRev) ON DELETE CASCADE
);
...
CREATE TABLE topicosRevisores (
    ...
    CONSTRAINT FK_topicosRevisores_RutRev FOREIGN KEY (RutRev) REFERENCES revisores(RutRev) ON DELETE CASCADE
);
...
CREATE TABLE autoresArticulos (
    ...
    CONSTRAINT FK_autoresArticulos_RutAut FOREIGN KEY (RutAut) REFERENCES autores(RutAut) ON DELETE CASCADE
);
